...
export default function Perfil() {
  const [perfil, setPerfil] = useState<PerfilData>({
    ...
    cpf_cnpj: '', endereco: '', logo: '', assinatura: '', cor_orcamento: 'azul',
  })
  ...
  useEffect(() => {
    api.get('/api/perfil/')
      ...
      .catch(() => {}) // se não existir ainda, usa defaults
  ...
  const set = (campo: keyof PerfilData, valor: string) =>
      ...
      <div className="space-y-5">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5">
          ...
          {perfil.logo ? (
            <div className="flex items-center gap-4">
              ...
              <div>
                <button
                  ...
                  className="flex items-center gap-2 text-sm font-semibold text-blue-600 hover:text-blue-700 mb-2 transition-colors">
            ...
            <div
              ...
              <p className="text-xs text-gray-400 mt-0.5">PNG, JPG ou SVG — o logo aparecerá no cabeçalho dos PDFs</p>
            </div>
          )}
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5">
          ...
          {perfil.assinatura ? (
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 mb-2">Será incluída automaticamente nos PDFs gerados</p>
                <button
                  onClick={() => assinaturaInputRef.current?.click()}
                  className="flex items-center gap-2 text-sm font-semibold text-blue-600 hover:text-blue-700 mb-1 transition-colors">
                  <Upload size={14} /> Alterar assinatura
                </button>
                <button
                  onClick={() => set('assinatura', '')}
                  className="flex items-center gap-1.5 text-xs font-semibold text-red-500 hover:text-red-600 transition-colors">
                  <X size={12} /> Remover assinatura
                </button>
              </div>
            </div>
          ) : (
            <div
              onClick={() => assinaturaInputRef.current?.click()}
              onDrop={e => handleDrop(e, 'assinatura')}
              onDragOver={e => e.preventDefault()}
              className="border-2 border-dashed border-gray-200 rounded-xl p-6 text-center cursor-pointer hover:border-blue-400 hover:bg-blue-50/30 transition-all">
          ...
          <input
            ...
            accept="image/*"
            className="hidden"
            onChange={e => { const f = e.target.files?.[0]; if (f) handleImageUpload(f, 'assinatura') }}
          />
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5">
          ...
          <p className="text-xs text-gray-400 mb-4">Escolha a cor dos detalhes do seu orçamento (cabeçalho da tabela, totais, etc.)</p>
          <div className="flex items-center gap-3 flex-wrap">
  {CORES.map(cor => (
  <button
    key={cor.id}
    onClick={() => set('cor_orcamento', cor.id)}
    className={`flex flex-col items-center gap-1.5 p-2 rounded-xl transition-all ${
      perfil.cor_orcamento === cor.id ? 'bg-gray-100' : 'hover:bg-gray-50'
    }`}>
    <div
      className="w-9 h-9 rounded-full shadow-sm"
      style={{
        backgroundColor: cor.hex,
        outline: perfil.cor_orcamento === cor.id ? `3px solid ${cor.hex}` : 'none',
        outlineOffset: '3px'
      }} />
        ...
        <div className="flex justify-end">
          <button
            ...
            className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-semibold text-white transition-all shadow-sm
              ${salvo ? 'bg-green-600' : 'bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400'}`}>
            {salvando
              ? <><Loader2 size={15} className="animate-spin" /> Salvando...</>
              : salvo
              ? <><Save size={15} /> Salvo com sucesso!</>
              : <><Save size={15} /> Salvar</>
            }
          </button>
        </div>
