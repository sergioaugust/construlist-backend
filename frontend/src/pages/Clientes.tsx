...
export default function Clientes() {
  ...
  // ── Buscar cidades via IBGE quando estado mudar ─────────────────────────────
  useEffect(() => {
    if (!form.estado) { setCidades([]); return }
    const sigla = form.estado
    ...
    fetch(`https://servicodados.ibge.gov.br/api/v1/localidades/estados/${sigla}/municipios?orderBy=nome`)
      ...
      .then((data: IBGEMunicipio[]) => {
        setCidades(data.map(m => m.nome.toUpperCase()))
      })
      .catch(() => setCidades([]))
  ...
  const buscarCEP = async (cep: string) => {
    ...
    try {
      ...
      if (!data.erro) {
        // Encontra o estado pela UF retornada
        const estadoEncontrado = ESTADOS.find(e => e.sigla === data.uf)
        setForm(f => ({
          ...f,
          endereco: data.logradouro?.toUpperCase() || f.endereco,
          bairro: data.bairro?.toUpperCase() || f.bairro,
          estado: estadoEncontrado?.sigla || f.estado,
          // cidade será preenchida após carregar as cidades do estado
        }))
        // Aguarda um tick para as cidades carregarem e então preenche
        if (estadoEncontrado) {
          setTimeout(() => {
            setForm(f => ({ ...f, cidade: data.localidade?.toUpperCase() || '' }))
          }, 800)
        }
      }
    } catch {
  ...
  const handleTelefone = (v: string) => handleChange('telefone', fmtTel(v))
  const handleCEP = (v: string) => {
    const fmt = fmtCEP(v)
    handleChange('cep', fmt)
    if (fmt.replace(/\D/g, '').length === 8) buscarCEP(fmt)
  }
  const handleCPFCNPJ = (v: string) => handleChange('cpf_cnpj', fmtCPFCNPJ(v))
  ...
  const salvar = async () => {
    ...
    try {
      if (editando) {
        await api.patch(`/api/clientes/${editando.id}/`, form)
      } else {
      ...
      {loading ? (
        ...
        <div className="grid gap-3">
          {filtrados.map(c => (
            <div
              key={c.id}
              className="bg-white rounded-2xl border border-gray-100 p-4 hover:shadow-md transition-all flex items-center gap-4 group"
            >
              {/* Avatar */}
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap gap-x-4 gap-y-0.5 mt-0.5">
                  ...
                  {(c.cidade || c.estado) && (
                    <span className="flex items-center gap-1 text-xs text-gray-500">
                      <MapPin size={11} /> {[c.cidade, c.estado].filter(Boolean).join(' - ')}
                    </span>
                  )}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <Field label="Bairro">
                    <input
                      ...
                      placeholder="Bairro"
                      value={form.bairro}
                      onChange={e => handleChange('bairro', e.target.value.toUpperCase())}
                      className={inputCls()}
                    />
                  </Field>
                </div>
                ...
                <Field label="Complemento">
                  <input
                    ...
                    onChange={e => handleChange('complemento', e.target.value.toUpperCase())}
                    className={inputCls()}
                  />
                </Field>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <Field label="Estado" error={errors.estado}>
                    <select
                      ...
                      className={inputCls(errors.estado) + ' cursor-pointer'}
                    >
                  ...
                  <Field label="Cidade" error={errors.cidade}>
                    <div className="relative">
                      <select
                        ...
                        className={`${inputCls(errors.cidade)} cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed`}
                      >
                        <option value="">
                          {loadingCidades
                            ? 'Carregando...'
                      ...
                      {loadingCidades && (
                        <Loader2 size={14} className="absolute right-8 top-1/2 -translate-y-1/2 text-blue-500 animate-spin pointer-events-none" />
                      )}
                    </div>
                  </Field>
                </div>
